...
macro_rules! ffi_impl_unit_for_number {
    ($obj_type: ident, $method_prefix: ident) => {
        paste::item! {
            ...
            #[no_mangle]
            pub unsafe extern "C" fn [< $method_prefix _abs >] (ptr: *mut $obj_type) -> *mut $obj_type {
                let left_obj = &*ptr;
                return Box::leak(Box::new(left_obj.abs()));
            }

            #[no_mangle]
            pub unsafe extern "C" fn [< $method_prefix _powi >] (ptr: *mut $obj_type, n: i32) -> *mut $obj_type {
                return Box::leak(Box::new((&*ptr).powi(n)));
            }

            #[no_mangle]
            pub unsafe extern "C" fn [< $method_prefix _powf >] (ptr: *mut $obj_type, n: f64) -> *mut $obj_type {
                return Box::leak(Box::new((&*ptr).powf(n)));
            }

            #[no_mangle]
            pub unsafe extern "C" fn [< $method_prefix _sqrt >] (ptr: *mut $obj_type) -> *mut $obj_type {
                return Box::leak(Box::new((&*ptr).sqrt()));
            }

            #[no_mangle]
            pub unsafe extern "C" fn [< $method_prefix _sin >] (ptr: *mut $obj_type) -> *mut $obj_type {
                return Box::leak(Box::new((&*ptr).sin()));
            }

            #[no_mangle]
            pub unsafe extern "C" fn [< $method_prefix _cos >] (ptr: *mut $obj_type) -> *mut $obj_type {
                return Box::leak(Box::new((&*ptr).cos()));
            }

            #[no_mangle]
            pub unsafe extern "C" fn [< $method_prefix _tan >] (ptr: *mut $obj_type) -> *mut $obj_type {
                return Box::leak(Box::new((&*ptr).tan()));
            }

            #[no_mangle]
            pub unsafe extern "C" fn [< $method_prefix _sinh >] (ptr: *mut $obj_type) -> *mut $obj_type {
                return Box::leak(Box::new((&*ptr).sinh()));
            }

            #[no_mangle]
            pub unsafe extern "C" fn [< $method_prefix _cosh >] (ptr: *mut $obj_type) -> *mut $obj_type {
                return Box::leak(Box::new((&*ptr).cosh()));
            }

            #[no_mangle]
            pub unsafe extern "C" fn [< $method_prefix _tanh >] (ptr: *mut $obj_type) -> *mut $obj_type {
                return Box::leak(Box::new((&*ptr).tanh()));
            }

            #[no_mangle]
            pub unsafe extern "C" fn [< $method_prefix _asin >] (ptr: *mut $obj_type) -> *mut $obj_type {
                return Box::leak(Box::new((&*ptr).asin()));
            }

            #[no_mangle]
            pub unsafe extern "C" fn [< $method_prefix _acos >] (ptr: *mut $obj_type) -> *mut $obj_type {
                return Box::leak(Box::new((&*ptr).acos()));
            }

            #[no_mangle]
            pub unsafe extern "C" fn [< $method_prefix _atan >] (ptr: *mut $obj_type) -> *mut $obj_type {
                return Box::leak(Box::new((&*ptr).atan()));
            }

            #[no_mangle]
            pub unsafe extern "C" fn [< $method_prefix _atan2 >] (ptr: *mut $obj_type, other_ptr: *mut $obj_type) -> *mut $obj_type {
                let other = &*other_ptr;
