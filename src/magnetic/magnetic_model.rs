...
#[no_mangle]
unsafe extern "C" fn magnetic_magnetic_model_new(
    epoch: f64,
...
#[no_mangle]
unsafe extern "C" fn magnetic_magnetic_model_from_file(filename: *const std::ffi::c_char, error_string: *mut *const std::ffi::c_char) -> *mut MagneticModel {
...
#[no_mangle]
unsafe extern "C" fn magnetic_magnetic_model_coeffs(ptr: *mut MagneticModel, n: usize, m: usize) -> *mut MagneticModelCoefficients {
    let obj = &*ptr;
...
#[no_mangle]
unsafe extern "C" fn magnetic_magnetic_model_name(ptr: *mut MagneticModel) -> *const std::ffi::c_char {
    return rust_str_to_c_str((&*ptr).name.clone());
}

#[no_mangle]
unsafe extern "C" fn magnetic_magnetic_model_set_name(ptr: *mut MagneticModel, new_name: *const std::ffi::c_char) {
    (*ptr).name = c_str_to_rust_str(new_name);
}

#[no_mangle]
unsafe extern "C" fn magnetic_magnetic_model_release_date(ptr: *mut MagneticModel) -> InteropDateStruct {
    return rust_naive_date_to_struct(&(&*ptr).release_date);
}

#[no_mangle]
unsafe extern "C" fn magnetic_magnetic_model_set_release_date(ptr: *mut MagneticModel, new_release_date: InteropDateStruct) {
    (*ptr).release_date = struct_to_rust_naive_date(new_release_date).unwrap();
}

#[no_mangle]
unsafe extern "C" fn magnetic_magnetic_model_calculate_field(ptr: *mut MagneticModel, point_ptr: *mut GeoPoint, date: InteropDateStruct) -> *mut MagneticField {
    let obj = &*ptr;
    let point = &*point_ptr;
